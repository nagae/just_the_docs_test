...
## 時点の長さと連続複利
風呂水の式\eqref{eq:bathtub}とキャッシュ・フロー流列の現在価値の式\eqref{eq:discounted_annual_cash_flow}を比べると，
前者は和記号の中に時点の長さ$\Delta t$が入っていたのに対し，後者は時点の長さが入っていない．
これは，$F(t)$が「$t$年目の期初時間の長さ（$\Delta t = 1\text{年間}$）を
